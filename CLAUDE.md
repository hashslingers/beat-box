...
- **Mix bus topology** (`initAudio`): drums and drone run on **separate buses** so drum hits never duck the drone (no shared compressor). Drums → gentle `compressor` (ratio ~5) → `drumBusGain` → master; drone → `droneBus` → `droneComp` → `droneBusGain` → master. Master = makeup `masterGain` → **subsonic/DC high-pass** (`masterHP`, 30 Hz, Q 0.707) → memoryless **tanh soft-clip** (`makeSoftClipCurve`, `masterLimiter`, no time constant so it cannot pump) → destination. The HPF exists because the kick sweeps to ~0 Hz, dumping ~56% of drum energy + a DC offset below 30 Hz (inaudible, wasted headroom); removing it also made the drum RMS honest, which is why the drone trims carry a global ~−2 dB vs their first calibration. Spectrum/EQ is analyzed offline with an FFT over the `__bbOffline` render (1/3-octave bands + sub-30 % + DC) — tonal-shape changes beyond the subsonic cleanup are aesthetic and were left to the user. Loudness comes from gain-staging into the soft clip; tune via measurement, not by ear. NB the soft clip is a *shared* nonlinearity: cross-talk is eliminated at the bus level, with a small peak-only residual (~0.5 dB on hard hits, no pumping) at the master.
- **Per-source drone normalization**: the 5 drone voices had wildly different native levels (synths ~+4 dB over drums, recorded `drones.mp3` ~16 dB under), so equal fader % meant different loudness and the drone didn't track the drums. `DRONE_SOURCE_TRIM` (a measured table, applied via `droneSrcGain()` in `startDrone` + `updateDroneVolume`) normalizes every source to land ~1–2 dB under the drums at matched fader positions. `droneComp` is intentionally gentle (ratio ~2.5) so the trims behave ~linearly and the fader doesn't saturate early. Additionally, the recorded `drones.mp3` holds a separate ~5-min drone per key, mastered up to ~11 dB apart, so `DRONE_KEY_TRIM` (applied in `playCustomDrone`/`updateDroneVolume`) levels the 12 keys to within ~0.6 dB. Re-measure offline (peak ≤ ~−1.5 dBFS, 0 clips, drone-vs-drum offset, switch-jump spread, per-key spread) after touching any of these.
